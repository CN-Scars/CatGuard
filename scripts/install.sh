...
download_dmg() {
  ...
  for asset in "${candidates[@]}"; do
    ...
    out="${WORKDIR}/${asset}"
    info "尝试下载：${asset}"
    # 优先匿名 curl（public 仓库）；失败时若有已认证的 gh 则回退（兼容 private 仓库 / 开发测试）。
    ...
    if command -v gh >/dev/null 2>&1 \
       && gh release download "$tag" --repo "$REPO" --pattern "$asset" --output "$out" --clobber 2>/dev/null; then
      DMG_PATH="$out"
      ok "下载完成（经 gh 认证）：${asset}"
      return 0
    fi
    warn "未找到或下载失败：${asset}，尝试下一个候选…"
  done
  die "无法下载 ${tag} 的可用 dmg（已尝试：${candidates[*]}）。若为 private 仓库，请先 gh auth login，或将仓库设为 public。"
}
